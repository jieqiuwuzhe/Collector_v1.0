...
#define tskIDLE_STACK_SIZE	configMINIMAL_STACK_SIZE
	...
	#define tpos_yield_IF_USING_PREEMPTION()
#else
	#define tpos_yield_IF_USING_PREEMPTION() portYIELD_WITHIN_API()
#endif
...
PRIVILEGED_DATA static volatile UBaseType_t uxTopReadyPriority 		= TPOS_TASKPROPERTY_SYS;
...
TCB_t * pxNewTCB;
	...
	if( xReturn == pdPASS )
	{
		if( xSchedulerRunning != pdFALSE )
		{
			...
			if( pxCurrentTCB->uxPriority < uxPriority )
			{
				tpos_yield_IF_USING_PREEMPTION();
			}
			else
			{
				mtCOVERAGE_TEST_MARKER();
...
#if ( INCLUDE_tpos_delayUntil == 1 )

	void tpos_delayUntil( TickType_t * const pxPreviousWakeTime, const TickType_t xTimeIncrement )
	{
...
#endif /* INCLUDE_tpos_delayUntil */
/*-----------------------------------------------------------*/

#if ( INCLUDE_vTaskDelay == 1 )

	void tpos_delay( const TickType_t xTicksToDelay )
	{
	TickType_t xTimeToWake;
...
/*-----------------------------------------------------------*/
	...
	UBaseType_t uxSavedInterruptStatus;
		...
		is defined in tposConfig.h then
...
void tpos_start( void )
{
BaseType_t xReturn;


		/* Create the idle task without storing its handle. */
		xReturn = tpos_createTask( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( TPOS_TASKPROPERTY_SYS | portPRIVILEGE_BIT ), NULL );  /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
...
/*----------------------------------------------------------*/
	...
	TickType_t xReturn;
		...
		if( pxCurrentTCB->uxPriority > TPOS_TASKPROPERTY_SYS )
		{
			xReturn = 0;
		}
		else if( listCURRENT_LIST_LENGTH( &( pxReadyTasksLists[ TPOS_TASKPROPERTY_SYS ] ) ) > 1 )
		{
	...
	UBaseType_t uxTask = 0, uxQueue = configMAX_PRIORITIES;
		...
		vTaskSuspendAll();
		{
			...
			if( uxArraySize >= uxCurrentNumberOfTasks )
			{
				...
				} while( uxQueue > ( UBaseType_t ) TPOS_TASKPROPERTY_SYS ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
		...
		#if ( ( configUSE_PREEMPTION == 1 ) && ( configIDLE_SHOULD_YIELD == 1 ) )
		{
			...
			if( listCURRENT_LIST_LENGTH( &( pxReadyTasksLists[ TPOS_TASKPROPERTY_SYS ] ) ) > ( UBaseType_t ) 1 )
			{
				tpos_yield();
			}
			else
...
/*----------------------------------------------------------*/
	...
	uint32_t ulTotalTime, ulStatsAsPercentage;
		...
		#if( configUSE_TRACE_FACILITY != 1 )
		{
			#error configUSE_TRACE_FACILITY must also be set to 1 in tposConfig.h to use vTaskGetRunTimeStats().
		}
		#endif
