...
void Radio_PowerUp(void);//not use
void  Radio_Init(void);//not use

void Radio_StartRx(uint8_t lens, uint8_t channel);//进入正常接收状态
void Radio_LDC_StartRx(uint8_t lens, uint8_t channel);//进入LDC模式接收状态

void  Radio_StartTx(uint8_t *data,uint8_t lens, uint8_t channel,uint8_t ldctx_en); //发送8位数据包
void  Radio_LongTx(uint8_t *data,uint32_t lens, uint8_t channel,uint8_t ldctx_en); //发送32位数据长包
void Radio_Tx_Repeat(uint8_t *data,uint8_t lens,uint32_t cnt, uint8_t channel,uint8_t ldctx_en); //发送8位数据包，重复发送相同包

uint8_t   Radio_CheckReceived(void); //检查是否接受到数据
uint8_t   Radio_CheckTransmitted(void); //检查是否发送完数据

uint8_t Radio_Cmd(uint8_t *data,uint8_t lens);
void Enter_LDCRX_mode(uint8_t osc_cal); //进入LDC接收模式
void Exit_LDCRX_mode(uint8_t osc_cal);   //离开LDC接收模式
void Enter_LDCTX_Mode(uint8_t osc_cal);  // 进入LDC发送模式
void Exit_LDCTX_Mode(uint8_t osc_cal); // 离开LDC发送模式
