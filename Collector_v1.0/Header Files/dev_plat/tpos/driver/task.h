...
#ifndef INC_TPOS_H
	#error "include tpos.h must appear in source files before include task.h"
#endif
...
typedef struct xTASK_STATUS
{
	...
	UBaseType_t uxBasePriority;		/* The priority to which the task will return if the task's current priority has been inherited to avoid unbounded priority inversion when obtaining a mutex.  Only valid if configUSE_MUTEXES is defined as 1 in tposConfig.h. */
	uint32_t ulRunTimeCounter;		/* The total run time allocated to the task so far, as defined by the run time stats clock.  See http://www.freertos.org/rtos-run-time-stats.html.  Only valid when configGENERATE_RUN_TIME_STATS is defined as 1 in tposConfig.h. */
 ...
 * \ingroup TaskUtils
 */
#define TPOS_TASKPROPERTY_SYS			( ( UBaseType_t ) 0U )
 ...
 * \ingroup SchedulerControl
 */
#define tpos_enterCriticalSection()		portENTER_CRITICAL()
 ...
 * \ingroup SchedulerControl
 */
#define tpos_leaveCriticalSection()			portEXIT_CRITICAL()
 ...
 * \ingroup Tasks
 */
#define tpos_createTask( pvTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask ) xTaskGenericCreate( ( pvTaskCode ), ( pcName ), ( usStackDepth ), ( pvParameters ), ( uxPriority ), ( pxCreatedTask ), ( NULL ), ( NULL ) )
 ...
 * \ingroup TaskCtrl
 */
void tpos_delay( const TickType_t xTicksToDelay ) PRIVILEGED_FUNCTION;
 ...
 * \ingroup SchedulerControl
 */
void tpos_start( void ) PRIVILEGED_FUNCTION;
 ...
 * set to 1 in tposConfig.h for pcTaskGetTaskName() to be available.
 *
 ...
 *
 * INCLUDE_uxTaskGetStackHighWaterMark must be set to 1 in tposConfig.h for
...
tpos.h.  When this is done TaskHookFunction_t will not yet have been defined,
 ...
 * INCLUDE_xTaskGetIdleTaskHandle is set to 1 in tposConfig.h.
 *
...
/**
 * configUSE_TRACE_FACILITY must be defined as 1 in tposConfig.h for
 ...
 * tposConfig.h then *pulTotalRunTime is set by uxTaskGetSystemState() to the
